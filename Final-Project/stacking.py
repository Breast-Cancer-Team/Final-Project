    ...
    try:
        if system_argumets[1] == "-stacking":
            val = input(
                        ...
                        "(noted: must be a csv file"
                        "organized with 7 features):")
            print("starting"
        ...
        print(str(e))
        print("Either illegal arguments or"
              "no arguments were given by the User."
              "Please read Reamde file")
...
def stacking_predictor(row):
    ...
    Define what our base layer will be composed of and
    then build a stacking classifier base
    on these models.
    ...
    x = our_trained_data[['radius_mean',
                          'texture_mean',
                          'area_mean',
                          'concavity_mean',
                          'concave points_mean',
    ...
    Stacking_classifier =
    StackingClassifier(estimator=estimators,
...
def please_predict_me(data):
    '''
    Takes parsed dataset and returns predictions for
    each patient as a dictionary. Output is displayed on main.
    '''
